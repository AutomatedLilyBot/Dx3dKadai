...
const char* psProfile)
{
    ...
    if (FAILED(device->CreateInputLayout(layout, layoutCount, vsb->GetBufferPointer(), vsb->GetBufferSize(), m_layout.GetAddressOf())))
        return false;
...
bool ShaderProgram::compileFromFile(ID3D11Device *device, const std::wstring &hlslPath,
                                    const D3D11_INPUT_ELEMENT_DESC *layout, UINT layoutCount, LPCSTR vsEntry, LPCSTR psEntry,
